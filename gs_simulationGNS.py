...
sys.path.append("PhysGaussian/gaussian-splatting")
sys.path.append("PhysGaussian")
...
from gs_rollout_GNS import predict
...
def transform2originNo(position_tensor):
    # 平行移動もスケーリングもしない
    scale = torch.tensor(1.0, device="cuda")  # スケールを1に設定
    original_mean_pos = torch.zeros_like(position_tensor[0], device="cuda")  # 平行移動をゼロベクトルに
    new_position_tensor = position_tensor  # 入力をそのまま返す
...
if __name__ == "__main__":
    ...
    parser.add_argument("--noise_tensor_std", type=float, default=6.7e-9)
    ...
    transformed_pos, scale_origin, original_mean_pos = transform2originNo(rotated_pos)
    ...
    for frame in tqdm(range(frame_num)):
        ...
        if args.render_img:
            pos = x_list[frame * step_per_frame + 1]
            cov3D = cov_list[frame * step_per_frame + 1]
            rot = R_list[frame * step_per_frame + 1]
